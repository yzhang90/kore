...
notFree sVar var p = go (toPattern p)
  where go (Variable sVar' var') = sVar' == sVar && var' == var
...
instance (IsSignature sig, Eq (Sort sig)) =>
  FromPattern (Maybe (WFPattern sig var)) (Sort sig) (Label sig) var where
  fromPattern = sequenceA >=> checkSorts1
instance (Applicative f) =>
  FromPattern (f (Pattern sort label var)) sort label var where
...
-- | This is a helper function for making constructors that fill
-- in the sort parameter based on the sorts of the arguments.
autoSort :: ( IsSignature sig
         ...
         -> (Sort sig -> m p -> b)
         -> (m p -> b)
autoSort k op arg1 = k (do s <- patternSort . toPattern <$> arg1; return (op s arg1))

arg :: (Monad m) => (m x -> x) -> m (m a -> x) -> m a -> x
arg outX mf ma = outX (do f <- mf; return (f ma))
...
andP', orP', impliesP', iffP'  :: (IsSigPatternIn m p sig var)
      => m p -> m p -> m p
andP' = autoSort (arg join) andP
orP' = autoSort (arg join) orP
impliesP' = autoSort (arg join) impliesP
iffP' = autoSort (arg join) iffP

notP' :: (IsSigPatternIn m p sig var)
      => m p -> m p
notP' = autoSort join notP

forallP' :: (IsSigPatternIn m p sig var)
         => Sort sig -> var -> m p -> m p
forallP' sVar var = autoSort join (\s -> forallP s sVar var)

existsP' :: (IsSigPatternIn m p sig var)
         => Sort sig -> var -> m p -> m p
existsP' sVar var = autoSort join (\s -> existsP s sVar var)
